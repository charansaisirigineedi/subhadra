...
session_start();
include "check.php";
$sql = mysqli_query($con,"select eor.id as id, eor.token_id as token_id, ppi.name as name, ppi.patient_phone_number 
as patient_phone_number , pif.type_of_inpatient as toi from  patient_surgery_form eor, patient_primary_information
 ppi,patient_inpatient_form pif where   eor.id = ppi.id  and pif.token_id=eor.token_id order by eor.time desc");
?>
											...
											<tbody>
												<?php
                                                    while($run = mysqli_fetch_assoc($sql))
                                                    {
                                                        ...
                                                        <td><a href="billing.php?pid='.$run['id'].'&tid='.$run['token_id'].'" class="btn btn-primary">ADD NEW CHARGE TO BILL</a></td>
                                                        </tr>';
                                                    }
                                                ?>
											</tbody>
